...
class ACNet(nn.Module):
    ...
    def encoder(self, rgb, depth):
        ...
        rgb = self.conv1(rgb)#2 64 240 320
        rgb = self.bn1(rgb)#2 64 240 320
        rgb = self.relu(rgb)#2 64 240 320
        depth = self.conv1_d(depth)#2 64 240 320
        depth = self.bn1_d(depth)#2 64 240 320
        depth = self.relu_d(depth)#2 64 240 320
        # print('!!!!! ', rgb.shape)
        atten_rgb = self.atten_rgb_0(rgb)#2 64 1 1
        atten_depth = self.atten_depth_0(depth)#2 64 1 1
        m0 = rgb.mul(atten_rgb) + depth.mul(atten_depth)#2 64 240 320

        rgb = self.maxpool(rgb)# 2 64 120 160
        depth = self.maxpool_d(depth)# 2 64 120 160
        m = self.maxpool_m(m0)#2 64 240 320

        # block 1
        rgb = self.layer1(rgb)# 2 256 120 160
        depth = self.layer1_d(depth)# 2 256 120 160
        m = self.layer1_m(m)#2 256 120 160

        atten_rgb = self.atten_rgb_1(rgb)#2 256 1 1
        atten_depth = self.atten_depth_1(depth)#2 256 1 1
        m1 = m + rgb.mul(atten_rgb) + depth.mul(atten_depth)#2 256 120 160

        # block 2
        rgb = self.layer2(rgb)#2 512 60 80
        depth = self.layer2_d(depth)#2 512 60 80
        m = self.layer2_m(m1)#2 512 60 80

        atten_rgb = self.atten_rgb_2(rgb)#2 512 1 1
        atten_depth = self.atten_depth_2(depth)#2 512 1 1
        m2 = m + rgb.mul(atten_rgb) + depth.mul(atten_depth)#2 512 60 80

        # block 3
        rgb = self.layer3(rgb)#2 1024 30 40
        depth = self.layer3_d(depth)#2 1024 30 40
        m = self.layer3_m(m2)#2 1024 30 40

        atten_rgb = self.atten_rgb_3(rgb)#2 1024 1 1
        atten_depth = self.atten_depth_3(depth)#2 1024 1 1
        m3 = m + rgb.mul(atten_rgb) + depth.mul(atten_depth)#2 1024 30 40

        # block 4
        rgb = self.layer4(rgb)#2 2048 15 20
        depth = self.layer4_d(depth)##2 2048 15 20
        m = self.layer4_m(m3)#2 2048 15 20

        atten_rgb = self.atten_rgb_4(rgb)#2 2048 1 1
        atten_depth = self.atten_depth_4(depth)#2 2048 1 1
        m4 = m + rgb.mul(atten_rgb) + depth.mul(atten_depth)#2 2048 15 20
    ...
    def forward(self, rgb, depth, phase_checkpoint=False):
        with torch.no_grad():
            x_size = rgb.size()
            im_arr = rgb.cpu().numpy().transpose((0, 2, 3, 1)).astype(np.uint8)
